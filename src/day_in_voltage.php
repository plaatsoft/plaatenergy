...
function plaatenergy_day_in_voltage_page() {
	...
	$sql  = 'select timestamp, voltage_f1, voltage_f2, voltage_f3 from energy1_details where ';
	...
	while ($row = plaatenergy_db_fetch_object($result)) {
		...
		$data .= round($row->voltage_f1,2);
		$data .= ',';
		$data .= round($row->voltage_f2,2);
		$data .= ',';
		$data .= round($row->voltage_f3,2);
		$data .= ']';
	}		
	...
	$page .= '<h1>'.t('TITLE_VOLTAGE', plaatenergy_dayofweek($date), $day, $month, $year).'</h1>';
...
function plaatenergy_in_voltage() {
	...
	switch ($pid) {
		...
		case PAGE_DAY_IN_VOLTAGE:
			return plaatenergy_day_in_voltage_page();
			break;
	}
}
