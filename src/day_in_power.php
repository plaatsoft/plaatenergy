...
function plaatenergy_day_in_power_page() {
	...
	$sql  = 'select timestamp, power_f1, power_f2, power_f3 from energy1_details where ';
	...
	while ($row = plaatenergy_db_fetch_object($result)) {
		...
		$data .= round($row->power_f1,2);
		$data .= ',';
		$data .= round($row->power_f2,2);
		$data .= ',';
		$data .= round($row->power_f3,2);
		$data .= ']';
	}		
	...
	$page .= '<h1>'.t('TITLE_POWER', plaatenergy_dayofweek($date), $day, $month, $year).'</h1>';
...
function plaatenergy_in_power() {
	...
	switch ($pid) {
		...
		case PAGE_DAY_IN_POWER:
			return plaatenergy_day_in_power_page();
			break;
	}
}
